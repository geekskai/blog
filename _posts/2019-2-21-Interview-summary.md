...
3.如果强行让数组调用push方法,会根据length的属性值的index进行属性的扩充
...
####  对称加密和非对称加密的区别,以及用到的场景?

`我的答案:`

对称加密:就是加密的秘钥和解密的秘钥是一样的,利用某种方法是可以推到出来的;
非对称加密:就是加密的秘钥和解密的秘钥不相同,而且哪怕你知道了其中的一个秘钥,你也无法推导出另外一个秘钥,非对称加密是用一种特殊的算法进行加密的,他是由三个数学家发明的,他们的名字的首字母分别是RSA,所以运用最早的最广泛的非对称算法是 RSA算法,但是现在很多专业的算法人员已经不用RSA算法了(不是很可靠),之前是几百位,现在是一千位.专业的算法有ECC算法.
*用到的场景* https 比特币 区块连中的算法等
...
`每个对象都有一个原型对象,通过__proto__ 指针,指向上一个原型,并从上一个原型对象中继承它的属性和方法,同时原型
