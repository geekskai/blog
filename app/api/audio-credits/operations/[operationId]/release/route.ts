...
import { releaseAudioCreditOperation } from "@/lib/audio-credits/repository"
...
export async function POST(
  ...
  try {
    const result = await releaseAudioCreditOperation(userId, operationId)
    return result
      ? NextResponse.json(result)
      : NextResponse.json({ error: "Audio Credit operation was not found." }, { status: 404 })
  } catch (error) {
    console.error("Failed to release Audio Credit operation", error)
    return NextResponse.json({ error: "Audio Credits could not be released." }, { status: 503 })
  }
}
