    ...
    <section className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-md md:p-8">
      ...
      <dl className="grid gap-4 md:grid-cols-2">
        {facts.map((fact) => (
          <div
            key={fact.label}
            className="rounded-xl border border-white/10 bg-gradient-to-br from-fuchsia-500/10 to-orange-500/10 p-4"
          >
      ...
      <ol className="space-y-3 text-slate-200">
        {steps.map((step, index) => (
          <li key={step} className="flex items-start gap-3">
            <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-pink-500/20 text-sm font-semibold text-pink-300">
              {index + 1}
            </span>
            <span>
              <strong>{step}</strong>
...
export function FAQSection() {
  const t = useTranslations("SoundCloudArtworkDownloader")
  const faqs = [
    { q: t("faq_question_1"), a: t("faq_answer_1") },
    { q: t("faq_question_2"), a: t("faq_answer_2") },
    { q: t("faq_question_3"), a: t("faq_answer_3") },
    { q: t("faq_question_4"), a: t("faq_answer_4") },
  ]
