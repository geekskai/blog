...
import { useBandcampToMp3 } from "../hooks/useBandcampToMp3"
...
export function MainSection({ initialUrl }: { initialUrl?: string }) {
  ...
  const { isPasting, setIsPasting } = useUrl({
    setTrackUrl,
      ...
      <section className="grid grid-cols-1 gap-6">
        <section className="rounded-[28px] border border-white/10 bg-white/5 shadow-2xl backdrop-blur-sm">
          <div className="border-b border-white/10 px-5 py-4 sm:px-6">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-300">{t("workflow.description")}</p>
          </div>
          ...
          <div className="space-y-5 px-5 py-5 sm:px-6 sm:py-6">
            <div className="rounded-[24px] border border-cyan-400/15 bg-slate-950/70 p-4 shadow-xl backdrop-blur-sm sm:p-5">
              <div className="flex flex-col gap-4">
                      ...
                      <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        >
                          {loadingAction === "trackMp3"
                            ? t("actions.extractingMp3")
                            : t("actions.getMp3Link")}
                        </button>
                        <button
                          type="button"
                ...
                <p className="text-xs leading-6 text-slate-400">{t("workflow.manualHint")}</p>
                ...
                <div className="flex flex-col gap-4 sm:flex-row">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {result.primaryMp3Url ? (
                      ...
                      <p className="mt-4 text-sm text-slate-400">{t("result.noMp3Exposed")}</p>
                    )}
                  </div>
                </div>
                        ...
                        >
                          ...
                          <div className="mt-3 flex flex-wrap gap-3">
                            <button
                              type="button"
                              onClick={() =>
                                triggerDownload(item.url, result.filename, result.sourceUrl)
                              }
                              className={primaryButtonClass}
                            >
